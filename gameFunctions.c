...
void initializeBlankString(int length, char *string) {
  char *blankString = (char*) malloc(sizeof(char) * (strlen(string) + 1));
  strcpy(blankString, string);
  int x = strlen(string);
  for(int i = 0; i < x; i++) {
    strcpy(blankString[i], "_");
  }
  return;
}
...
int revealGuessedLetter(const char *solutionString, char *string, char guess) {
  int x = strlen(solutionString);
  for(int i = 0; i < x; i++) {
    if(strcmp(solutionString[i], guess) == 0) {
      if(strcmp(solutionString[i], string[i]) != 0) {
        strcpy(string[i], "_");
      } else if(strcmp(solutionString[i], string[i]) == 0) {
        strcpy(string[i], solutionString[i]);
        return 1;
      }
      }else if(strcmp(solutionString[i], guess) != 0) {
        strcpy(string[i], "_");
        return 0;
    }
  }
